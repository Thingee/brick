...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./brick/tests"
    fi
  ...
  if [ setup.cfg -nt brick.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='brick/*' --omit='brick/openstack/common/*' -d covhtml -i
  fi
...
function run_pep8 {
  ...
  bash -c "${wrapper} flake8 brick* bin/*"
}
